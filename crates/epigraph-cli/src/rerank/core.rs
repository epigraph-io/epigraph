...
impl RerankSummary {
    /// Counts of `per_pair_discards` by reason, keyed on the stable
...
/// (`crate::matching_client`) needs to test how each failure mode is reported,
/// and that must exercise the same code the batch loop runs.
...
async fn rerank_inner(
    ...
    if !breakdown.is_empty() {
        let rendered: Vec<String> = breakdown
            ...
            .collect();
        eprintln!("  no-verdict breakdown: {}", rendered.join("; "));
    }
