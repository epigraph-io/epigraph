...
/// the model was simply silent about. Carrying the reason out is what lets
/// `match_candidates.verifier_rationale` say *which* thing went wrong.
///
/// The variants split into two scopes, and the split is load-bearing because
/// the emitted string has to be true for the pair it is written on:
///
...
impl DiscardReason {
    ...
    /// about the individual pair the string is stored against.
