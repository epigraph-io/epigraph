...
pub fn collect_ddl(migrations_dir: &str, table: &str) -> Result<String> {
    ...
    let mut paths: Vec<_> = WalkDir::new(migrations_dir)
        ...
        .filter_map(|e| e.ok())
        .filter(|e| {
            e.file_type().is_file() && e.path().extension().and_then(|s| s.to_str()) == Some("sql")
        })
...
pub fn collect_call_sites(repo: &str, table: &str) -> Result<Vec<CallSite>> {
    ...
    for entry in WalkDir::new(&crates_dir).into_iter().filter_map(|e| e.ok()) {
        ...
        if p.components().any(|c| {
            c.as_os_str() == "target" || c.as_os_str() == ".sqlx" || c.as_os_str() == "migrations"
        }) {
            continue;
        }
...
pub fn extract_fk_targets(ddl: &str) -> Vec<String> {
    ...
    let mut out: Vec<String> = re.captures_iter(ddl).map(|c| c[1].to_lowercase()).collect();
    out.sort();
    out.dedup();
    out
}

pub fn build_dossier(repo_root: &str, migration_dir: &str, table: &TableRef) -> Result<Dossier> {
...
// `pub fn run` (the extract subcommand orchestrator) lives in `extract.rs` —
// it references sibling modules (`discover`, `llm`) which are not visible when
// dossier.rs is `#[path]`-included into the test crate.
