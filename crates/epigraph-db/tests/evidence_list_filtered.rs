...
async fn predicates_run_before_limit(pool: PgPool) {
    ...
    let rows = EvidenceRepository::list_filtered(&pool, &filter, 3, 0)
        .await
        .unwrap();
    assert_eq!(
        rows.len(),
    ...
    assert_eq!(
        EvidenceRepository::count_filtered(&pool, &filter)
            .await
            .unwrap(),
        1,
...
async fn count_and_list_agree_across_every_filter_combination(pool: PgPool) {
    ...
    for (label, filter, expected) in cases {
        let count = EvidenceRepository::count_filtered(&pool, &filter)
            .await
            .unwrap();
        ...
        let rows = EvidenceRepository::list_filtered(&pool, &filter, 100, 0)
            .await
            .unwrap();
...
async fn content_contains_excludes_null_raw_content(pool: PgPool) {
    ...
    assert_eq!(
        EvidenceRepository::count_filtered(&pool, &EvidenceListFilter::default())
            .await
            .unwrap(),
        2
    );
    ...
    let rows = EvidenceRepository::list_filtered(&pool, &filter, 10, 0)
        .await
        .unwrap();
...
async fn paging_is_stable_when_every_row_shares_a_timestamp(pool: PgPool) {
    ...
    for page in 0..5 {
        let rows = EvidenceRepository::list_filtered(&pool, &filter, 2, page * 2)
            .await
            .unwrap();
