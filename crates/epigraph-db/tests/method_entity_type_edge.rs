...
//!
//! Migration 101 seeds the missing row. These tests assert the *behaviour* that
...
/// Load-bearing control: deleting the seeded row reproduces the pre-101 failure.
///
/// This is what pins the two tests above to migration 101 rather than to some
...
/// the FK's, is the pre-101 symptom an operator would have seen. Either spelling
...
async fn method_edge_fails_when_the_registry_row_is_absent(pool: PgPool) -> sqlx::Result<()> {
    ...
    // Reproduce the pre-101 registry state. Safe on this throwaway
    ...
    assert_eq!(
        deleted, 1,
        "migration 101 must have seeded exactly one `method` row for this control \
...
/// 101's INSERT always takes the insert branch, so `ON CONFLICT … DO NOTHING`
...
/// DO NOTHING, 101 is a silent no-op on such a database and `is_core` stays
...
#[sqlx::test(migrations = "../../migrations")]
async fn migration_101_promotes_a_pre_existing_non_core_method_row(
    pool: PgPool,
) -> sqlx::Result<()> {
    /// The shipped migration, not a paraphrase of it.
    const MIGRATION_101: &str = include_str!("../../../migrations/101_seed_method_entity_type.sql");
    ...
    sqlx::raw_sql(MIGRATION_101).execute(&pool).await?;
    ...
    assert!(
        row.4,
        "101 must ASSERT is_core = true on a pre-existing non-core row, not skip it \
...
/// does: it runs on a virgin `#[sqlx::test]` database, where 101 always inserts.
/// The pre-existing-row case is covered by
/// `migration_101_promotes_a_pre_existing_non_core_method_row` above.
#[sqlx::test(migrations = "../../migrations")]
