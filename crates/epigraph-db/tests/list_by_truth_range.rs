...
async fn truth_range_filter_finds_matches_outside_recent_window(pool: PgPool) {
    ...
    let results = ClaimRepository::list_by_truth_range(&pool, &viewer, 0.0, 0.75, None, 20, 0)
        .await
        .unwrap();
    ...
    let high = ClaimRepository::list_by_truth_range(&pool, &viewer, 0.9, 1.0, None, 20, 0)
        .await
        .unwrap();
    assert!(
