...
async fn mark_duplicate_happy_path(pool: PgPool) {
    ...
    ClaimRepository::mark_duplicate(&pool, ClaimId::from_uuid(dup), ClaimId::from_uuid(canonical))
        .await.unwrap();
...
async fn mark_duplicate_rejects_already_superseded(pool: PgPool) {
    ...
    let err = ClaimRepository::mark_duplicate(&pool, ClaimId::from_uuid(dup), ClaimId::from_uuid(other_canonical))
        .await.err().unwrap();
...
async fn mark_duplicate_rejects_self(pool: PgPool) {
    ...
    let err = ClaimRepository::mark_duplicate(&pool, ClaimId::from_uuid(claim), ClaimId::from_uuid(claim))
        .await.err().unwrap();
...
async fn mark_duplicate_rejects_missing_canonical(pool: PgPool) {
    ...
    let err = ClaimRepository::mark_duplicate(&pool, ClaimId::from_uuid(dup), ClaimId::from_uuid(bogus))
        .await.err().unwrap();
