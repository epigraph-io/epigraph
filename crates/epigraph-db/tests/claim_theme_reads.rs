...
async fn summary_reports_live_member_count_not_the_stale_column(pool: PgPool) {
    ...
    let summary = ClaimThemeRepository::get_summary(&pool, theme)
        .await
        .expect("get_summary")
        .expect("theme exists");
...
async fn retired_members_are_excluded_from_count_and_page(pool: PgPool) {
    ...
    let members = ClaimThemeRepository::member_claim_ids(&pool, theme, 50, 0)
        .await
        .expect("member page");
...
async fn member_paging_walks_the_theme_to_exhaustion_without_repeats(pool: PgPool) {
    ...
    let mut offset = 0i64;
    loop {
        let page = ClaimThemeRepository::member_claim_ids(&pool, theme, 3, offset)
            .await
            .expect("member page");
        if page.is_empty() {
            break;
...
async fn list_summaries_filters_by_prefix_and_orders_by_live_count(pool: PgPool) {
    ...
    let page = ClaimThemeRepository::list_summaries(&pool, Some("px-"), 50, 0)
        .await
