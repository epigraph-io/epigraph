...
//! [`LlmError::NotAvailable`]. Concrete built-ins (Anthropic API, mock) and
//! private extensions (Claude CLI subprocess for subscription-prepaid OAuth)
//! register themselves at process startup via [`register_llm_provider`]; the
//! [`default_llm_provider`] helper walks the registry and returns the first
//! provider whose [`LlmProvider::is_active`] returns `true`.
//!
...
///
/// Implementors live in built-in crates (Anthropic, Mock) or private
/// extension crates (Claude CLI, vLLM, OpenAI). Each kernel-side caller
/// resolves an [`Arc<dyn LlmProvider>`] via [`default_llm_provider`] or
