...
async fn find_workflow_hierarchical_resolve_to_latest_includes_resolved_steps(
    ...
    let resp_default: serde_json::Value = reqwest::Client::new()
        .get(format!("http://{addr}/api/v1/workflows/hierarchical/search?q={unique_goal}"))
        .send().await.unwrap().json().await.unwrap();
    assert_eq!(resp_default["resolve_to_latest"], serde_json::json!(false));
    assert!(resp_default["workflows"].as_array().unwrap().iter().all(|w| w.get("resolved_steps").is_none()));
    ...
    assert!(workflows.iter().all(|w| w["resolved_steps"].is_array()),
