...
async fn themes_expand_returns_neighborhoods_for_seeded_theme() {
    ...
    for (id, content) in [(claim_a, "atom-a"), (claim_b, "atom-b")] {
        ...
        .bind(id).bind(content).bind(hash).bind(agent_id).bind(theme_id)
    ...
    let (lo, hi) = if nbr_a < nbr_b { (nbr_a, nbr_b) } else { (nbr_b, nbr_a) };
    sqlx::query(
        "INSERT INTO neighborhood_edges (run_id, neighborhood_a, neighborhood_b, weight) \
         VALUES ($1, $2, $3, 0.7)"
    ).bind(run_id).bind(lo).bind(hi).execute(&pool).await.unwrap();

    let (addr, _shutdown) = common::spawn_app(&url).await;
    let resp = reqwest::Client::new()
        .get(format!("http://{addr}/api/v1/graph/themes/{theme_id}/expand"))
        .header("Authorization", format!("Bearer {}", common::test_bearer_token()))
        .send().await.unwrap();
    assert_eq!(resp.status().as_u16(), 200, "themes/expand should return 200");
    let body: Value = resp.json().await.unwrap();
    assert_eq!(body["theme_id"].as_str().unwrap(), theme_id.to_string());
    let nbrs = body["neighborhoods"].as_array().unwrap();
    assert_eq!(nbrs.len(), 2, "expected exactly 2 neighborhoods for the seeded theme");
