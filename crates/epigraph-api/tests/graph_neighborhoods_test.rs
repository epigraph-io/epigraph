...
async fn neighborhoods_expand_compound_returns_compound_nodes_with_induced_edges() {
    ...
    assert_eq!(
        kinds.iter().filter(|k| **k == "compound").count(),
        2,
        "expected 2 compound nodes"
    );
    ...
    assert_eq!(
        induced.len(),
        1,
        "one cross-compound atom edge → one induced compound edge"
    );
...
async fn neighborhoods_expand_atomic_returns_atoms_and_compound_groups() {
    ...
    let resp = reqwest::Client::new()
        .get(format!(
            "http://{addr}/api/v1/graph/neighborhoods/{nbr}/expand?mode=atomic"
        ))
        .header(
            "Authorization",
            format!("Bearer {}", common::test_bearer_token()),
    ...
    assert_eq!(
        groups[0]["compound_id"].as_str().unwrap(),
