...
async fn theme_scope_excludes_off_theme_hits_on_hybrid() {
    ...
    let body = common::first_text(
        &recall_with_pgvec(&server, unscoped, Some(pgvec.clone()))
            .await
    ...
    .expect("params");
    let body = common::first_text(
        &recall_with_pgvec(&server, scoped, Some(pgvec.clone()))
            .await
    ...
    .expect("params");
    let body = common::first_text(
        &recall_with_pgvec(&server, by_label, Some(pgvec.clone()))
            .await
...
async fn theme_scope_holds_when_the_embedder_is_down() {
    ...
    let unscoped: RecallParams =
        serde_json::from_value(json!({ "query": token, "limit": 10, "min_truth": 0.0 }))
            .expect("params");
    let body = common::first_text(
        &recall_with_pgvec(&server, unscoped, None)
            .await
    ...
    .expect("params");
    let body = common::first_text(
        &recall_with_pgvec(&server, scoped, None)
            .await
...
async fn offset_walks_a_theme_to_exhaustion_without_repeats() {
    ...
    loop {
        ...
        .expect("params");
        let body = common::first_text(
            &recall_with_pgvec(&server, params, Some(pgvec.clone()))
                .await
                .expect("paged recall"),
        );
...
async fn include_workflows_with_a_theme_or_an_offset_is_rejected() {
    ...
    .expect("params");
    let err = recall_with_pgvec(&server, with_theme, Some(cluster_pgvec(3)))
        .await
    ...
    .expect("params");
    recall_with_pgvec(&server, with_offset, Some(cluster_pgvec(3)))
        .await
...
async fn unresolvable_theme_selectors_are_rejected_not_ignored() {
    ...
    for bad in [
        ...
        let outcome = recall_with_pgvec(&server, params, Some(pgvec.clone())).await;
        assert!(
            outcome.is_err(),
