...
async fn list_themes_reports_live_membership_and_pages_deterministically() {
    ...
    let body = common::first_text(&list_themes(&server, params).await.expect("list_themes"));
    ...
    let p2: ListThemesParams =
        serde_json::from_value(json!({ "label_prefix": prefix, "limit": 1, "offset": 1 }))
            .expect("params");
    let page1 = common::first_text(&list_themes(&server, p1).await.expect("page1"));
    let page2 = common::first_text(&list_themes(&server, p2).await.expect("page2"));
...
async fn get_theme_returns_member_ids_without_content() {
    ...
    let body = common::first_text(&get_theme(&server, params).await.expect("get_theme"));
