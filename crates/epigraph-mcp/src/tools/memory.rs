    ...
    // post-commit embedding. Spawned, so the caller does not wait on it.
    {
        ...
        let event = epigraph_db::NewRecallEvent {
            ...
            params: serde_json::json!({
                ...
                "since": params.since,
                // Same argument for the theme scope and the page offset: the
                // same query pinned to a theme, or taken at offset 20, returns
                // a different set, so both are part of what was asked and a
                // retrieval whose scope cannot be reconstructed from its audit
                // row is an unauditable retrieval. The RESOLVED theme id is
                // logged, not the raw selector, so a `theme_label` lookup stays
                // reconstructible after the label is renamed.
