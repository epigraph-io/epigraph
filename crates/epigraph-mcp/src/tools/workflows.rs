...
async fn find_workflow_post_embed(
    ...
    for (_, candidate) in candidates {
        ...
        match candidate {
            ...
            Candidate::Hierarchical(row) => {
                ...
                if let Some(r) =
                    hierarchical_workflow_result(&server.pool, &row, steps, &affinity_map).await
                {
                    results.push(r);
                }
            }
    ...
    if results.len() < half {
        ...
        for row in text_rows {
            ...
            if let Some(r) =
                hierarchical_workflow_result(&server.pool, &scored, steps, &affinity_map).await
            {
                results.push(r);
            }
        }
...
async fn hierarchical_workflow_result(
    ...
    let lineage_root = WorkflowRepository::find_lineage_root(pool, row.id)
        .await
        .unwrap_or_else(|e| {
