...
use epigraph_engine::epistemic_interval::{
    restrict_epistemic_frame_evidence, restrict_epistemic_negative, restrict_epistemic_positive,
    EpistemicInterval,
};
use epigraph_engine::sheaf::{restriction_kind_with_profile, RestrictionKind, RestrictionProfile};
...
///
/// Uses the existing `RestrictionKind` framework (per `epigraph-engine::sheaf`)
/// to map the edge's relationship onto a transmission factor, applies it to
/// the source's stored `EpistemicInterval`, materializes a BBA on the
/// canonical binary frame, persists with `perspective_id = edge_id`, then
/// re-combines all of the target's stored BBAs and updates its belief columns.
///
...
/// retry later once the source acquires a BBA.
...
/// Re-fetch all BBAs on (claim, frame), discount by source_strength, combine,
/// and write the resulting Bel/Pl/BetP/conflict/missing to the claims row.
/// Mirrors the cascade in `auto_wire_ds_update` post-store.
