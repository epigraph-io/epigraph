...
impl EpiGraphMcpFull {
    ...
    #[tool(
        description = "Submit an epistemic claim with evidence. The full evidence text is preserved for human audit. Supports all evidence types (empirical 1.0x, statistical 0.9x, logical 0.85x, testimonial 0.6x). Prefer this over memorize when you have a source or data to cite. The claim is authored by the calling agent (the authenticated caller over HTTP, this server's own agent on stdio) and owned by that agent's personal group, or by its operator's personal group when the agent is operator-linked: if an operator has revoked that personal-group membership, the call is refused and writes nothing. It never restores the membership; restoring it is an operator action. All-or-nothing: the claim, its evidence, reasoning trace, verb-edges and its Dempster-Shafer belief (BBA, frame assignment, cached belief and derived truth_value) commit together; if the belief cannot be wired the call fails and nothing is written, so a retry is safe. Only the embedding is best-effort after commit (embedded=false)."
    )]
    ...
    #[tool(
        description = "Atomically add and/or remove labels on an existing claim. Idempotent. Adding or removing the 'resolved' label requires ownership of the claim on EVERY transport: you authored it, or (stdio) you are an agent linked to the same operator as its author, or (HTTP) you are its author's operator; over HTTP a claims:admin token also passes. Anyone else is refused with nothing written. Every other label is ungated. With a claims:admin token (HTTP) a claim owned by a group you cannot write is written through the AUDITED ADMIN PATH instead: the write records you (the admin) as principal, never the claim's author, and a security_events audit row (claims.admin_write) names you, your token, the claim and the before/after state; it requires a live claims:admin grant on your token's own client record, and is refused with nothing written otherwise. The response's admin_path field says which path ran."
    )]
    ...
    #[tool(
        description = "Retire a backlog claim in one call: submits a resolution claim via the canonical submit_claim pipeline (idempotent create + Evidence + Trace + DERIVED_FROM/HAS_TRACE/AUTHORED edges + DS auto-wire + embedding), prefixed with 'Resolves <original_id>: ' and labeled ['resolved'], then patches the original claim's labels with add=['resolved'] (keeping 'backlog'). Label-side retirement — original stays is_current=true / supersedes=None. Optionally takes basis_claim_ids: the claims that justified the closure, each recorded as a `basis -justifies-> resolution` edge so a later retraction of a basis can be reverse-queried to find the closures resting on it (it does not reopen anything by itself). Returns {resolution_claim_id, original_id, original_labels, basis_claim_ids, basis_edge_ids}. All-or-nothing: the resolution claim, its justifies edges and the original's 'resolved' label commit together on one transaction with the calling agent (the authenticated caller over HTTP, this server's own agent on stdio)'s write authority, or nothing is written and the call fails (a retry is safe). An original owned by a group the calling agent cannot write (another agent's claim) is refused that way, unless the token carries claims:admin: then the resolution claim is still yours, in your own group, and only the original's 'resolved' label is written through the AUDITED ADMIN PATH (recorded with you as principal, plus a security_events claims.admin_write audit row; it requires a live claims:admin grant on your token's own client record). Only the resolution claim's embedding runs after commit, best-effort."
    )]
    ...
    #[tool(
        description = "Quick-store a memory as a testimonial claim (0.6x evidence weight). For facts you want to recall later. Tags are persisted as claim labels — queryable via `query_claims_by_label`. The claim is authored by the calling agent (the authenticated caller over HTTP, this server's own agent on stdio) and owned by that agent's personal group, or by its operator's personal group when the agent is operator-linked: if an operator has revoked that personal-group membership, the call is refused and writes nothing. It never restores the membership; restoring it is an operator action. All-or-nothing: the claim, its tags, evidence, trace and Dempster-Shafer belief commit together; if the belief cannot be wired the call fails and nothing is written. Only the embedding is best-effort after commit."
    )]
    async fn memorize(
        &self,
