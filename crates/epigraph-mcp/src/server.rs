...
impl EpiGraphMcpFull {
    ...
    #[tool(
        description = "Patch a claim atomically (trace_id, properties JSONB merge, label add/remove). FAST PATH — does NOT emit provenance. Use REST PATCH /api/v1/claims/:id if audit trail required. When the caller is authenticated (HTTP), the whole patch requires claims:admin or ownership of the claim, as PATCH /api/v1/claims/:id does. All-or-nothing: the whole patch lands or nothing does. A claim you cannot read is reported as not found. A claim owned by a group the calling agent (the authenticated caller over HTTP, this server's own agent on stdio) cannot write (another agent's claim) is refused with nothing written, even with claims:admin."
    )]
