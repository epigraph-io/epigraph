...
impl EpiGraphMcpFull {
    ...
    #[tool(
        description = "Record what actually happened when you used a workflow. For a workflows-table id (what `store_workflow` / `ingest_workflow` return) it delegates to `report_hierarchical_outcome` and has that tool's response and semantics: counters plus per-step rows, no evidence, no belief change, NOT idempotent, and execution_log[].step_index mapped to the steps in original plan order. Legacy flat workflow claim IDs are still supported: there the run is recorded as evidence plus a Dempster-Shafer truth update, all-or-nothing (an error writes nothing, so an identical retry is safe). On a PUBLIC flat workflow claim owned by a group this server's agent cannot write (most shared workflows), the outcome evidence and its BBA are owned by this agent's own group (its operator's group for an operated agent) and stay public, the claim's cached belief is recombined through an audited path when it carries binary_truth or has no cache (cache_written; warning otherwise), and the claim's truth_value is NOT written (truth_written=false, truth_after=truth_before). A group-private workflow claim this server's agent cannot write is refused."
    )]
