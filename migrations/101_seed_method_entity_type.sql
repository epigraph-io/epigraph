-- Migration 101: seed `method` as a CORE entity type (ADDITIVE, one row).
--
-- EVIDENCE
...
-- This file was authored as `094` and is renumbered to `101` here. TWO reasons,
-- and only the second was known when it was written: `092-099` is the SECOND
-- reserved tenancy block (claimed by the operator 2026-09-15, recorded in
-- migrations/README.md), and `094` collided with the deprecating internal-main's
-- DIFFERENT `094_stop_truth_value_overwrite.sql`. The renumber dissolves both.
-- It has never been applied to a deployed database — production is at 59 — so
-- the immutability rule for applied migrations does not bind it.
--
...
-- `tenancy_tier` IS DECLARED, NOT DEFAULTED, and it is the one field this file
-- did not carry when it was numbered 094 against a pre-tenancy `main`.
--
-- Migration 069 adds the column, seeds every type that existed at 069 time, and
-- then `ALTER COLUMN tenancy_tier DROP DEFAULT` — so an INSERT that omits it
-- raises `23502 null value in column "tenancy_tier"`, which is exactly what this
-- file did on a `#[sqlx::test]` database after the merge. 069's own header states
-- the rule it is enforcing: "a type registered after this migration must SAY what
-- tenancy shape its backing table has, and cannot be silent."
--
-- 'derived', NOT 'columns'. `public.methods` is absent from migration 062's
-- `tier_a` array, so it has neither `owner_group_id` nor `visibility`; 'columns'
-- asserts both are present and NOT NULL, and `routes/admin.rs`'s registration
-- handler refuses that tier for a table lacking them. 'derived' is the bucket
-- 069 swept every other non-tier-A kernel type into (paper, analysis, task,
-- workflow, …) and is the honest description: a method row is gated by whatever
-- tier-A row references it, not by tenancy columns of its own.
INSERT INTO entity_types (
    type_name, schema_name, table_name, id_column,
    is_optional, is_core, registered_by, tenancy_tier, description
)
VALUES (
    'method', 'public', 'methods', 'id',
    false, true, NULL, 'derived',
    'Research method entity; kernel-owned public.methods (migration 001). '
    'Seeded core by kernel migration 101, superseding episcience''s '
...
ON CONFLICT (type_name) DO UPDATE SET
    ...
    -- NOT overwritten from EXCLUDED. Every other column here is kernel-owned
    -- shape that this file is the authority on; `tenancy_tier` is a security
    -- classification an operator or a later migration may have TIGHTENED, and
    -- stamping 'derived' back over a hand-set 'columns' would silently relax it.
    -- COALESCE keeps the existing value and only fills a row that somehow has
    -- none.
    tenancy_tier  = COALESCE(entity_types.tenancy_tier, EXCLUDED.tenancy_tier),
