...
| `KANBAN_AGENT_DISALLOWED_TOOLS` | merge/admin `gh` patterns, `git push` (every form), `curl`, `wget`, backlog-mutating MCP tools | `--disallowedTools` for development agents |
...
- Agents do not push and do not open PRs. `Bash(git push:*)` is denied and the prompts say so; when a run ends, the board pushes the card's own `kanban/*` branch and opens the PR (see "The PR tree"). The board's git gets no tokens, so that push authenticates through **config files** (a git credential helper, `gh auth setup-git`, `~/.ssh`) or `SSH_AUTH_SOCK`. The board's own `gh` calls do get `GH_TOKEN` if you started it with one.
...
- `SSH_AUTH_SOCK` is not passed to agents by default. The board's own git gets it (see "What the board does to your repository"), so pushes over ssh with an agent work without allow-listing it.
...
- **Develop** works from `backlog`, from `review` (re-run with a fresh session), or on a `failed`/`stopped` card in `develop`. **Request changes** resumes the same Claude session (`--resume <session_id>`) in the same worktree; the agent commits and the board pushes the branch, which updates the same PR. **Stop** kills the agent's process group. **Reject** moves the card back to `backlog` and leaves its PR open. Pass `cleanup: true` to also remove the worktree.
...
2. Each card gets branch `kanban/<id8>-<slug>` in the worktree `$KANBAN_HOME/worktrees/<id8>`, based on the integration branch. The agent only commits. When the run ends, the board pushes that branch, and only that branch, with a non-force `refs/heads/<branch>:refs/heads/<branch>` refspec (a rewritten history is rejected by the remote and raised as a `Publishing failed:` blocker, never forced). If the run reported `done` and no PR exists yet, the board runs `gh pr create --base <integration> --head <branch>`, titled with the card title.
...
4. **Open integration PR** opens (or reuses) `integration -> main`. `gh pr list --head` also matches fork PRs from a branch of the same name, so lookups by head branch skip every hit with `isCrossRepository != false` (or a head owner other than the pinned repository's) before choosing one; a fork cannot squat the predictable integration branch name. It reuses an existing PR only after `gh pr view` confirms the PR is an open, same-repository PR from the integration branch into the configured base branch. A recorded PR that no longer matches is forgotten. The PR body lists the member PRs and backlog claim ids.
5. **Merge integration → main** gets the same checks as Accept (base, head, state, not a fork, head sha) and merges with `--match-head-commit`. There is no unpinned merge. A missing or malformed head sha is refused. All accepted cards move to `shipped`, and the next develop run starts a new integration branch. If "resolve backlog items" is checked (the default), a background `claude -p` run calls the EpiGraph MCP `resolve_backlog_item(original_id, resolution_content)` for each shipped card, citing both PRs. That run gets its own throwaway detached worktree at `<remote>/<base>` (`$KANBAN_HOME/worktrees/_retire-*`, removed afterwards), never your checkout. It has no built-in tools (`--tools ""`), only `KANBAN_RESOLVE_TOOL` pre-approved, and `--permission-mode dontAsk`. Its prompt carries only board-known fields (claim id, the validated PR URLs, the card title), each JSON-quoted; the development agent's summary is never passed on, because quoting stops a structural breakout but not a persuasive instruction to an LLM that holds `resolve_backlog_item` for any id. The board reads the run's `stream-json` event stream and counts a card as resolved only when it sees a `resolve_backlog_item` call for that id whose result was not an error. The agent's own "resolved" list is recorded, not believed. Calls for ids outside the shipped set are logged and flagged on each shipped card (`resolve_outside_shipped_set`). The result is recorded in each card's history (`backlog_resolved: true|false`).

**Both merges are bound to the head you saw.** `--match-head-commit` alone only covers the milliseconds between `gh pr view` and `gh pr merge`; the head it pins would otherwise be whatever GitHub reports when the click arrives, which an agent (same uid, same push credentials) can have moved after your review. So:

- The Accept dialog first calls `GET /api/cards/{id}/pr`, which returns the PR's current `head_sha` and `checks`, and shows them. Accept sends that sha back as `expected_head`.
- `GET /api/integration` returns `head_sha` next to `checks`, read in the same `gh pr view`; the Ship dialog shows it and sends it back as `expected_head`.
- The server re-verifies the PR and refuses with 409 `head_moved` (carrying the new `head_sha`) when the head differs from `expected_head`, or 409 `head_required` when the request names none. The UI then asks you to review again; it never retries with the new head on its own.

**CI is required on both merge paths.** Accept and Ship both refuse unless the PR's CI checks are `pass`. The check state is read in the same `gh pr view` that supplies the pinned head sha. `pending`, `fail` and `none` (no checks reported) all refuse. `pass` is an allow-list: a check run counts only as `COMPLETED` with conclusion `SUCCESS`, `NEUTRAL` or `SKIPPED`, a status context only as `SUCCESS`. `STALE`, a completed run with no conclusion, or any value the board does not know reads as `pending`; a rollup with no recognisable entry reads as `none`. Integration branches the board creates have no branch protection of their own, so set `KANBAN_REQUIRED_CHECKS` if a workflow can register its jobs late. The only way past is `"override_checks": true` (a JSON boolean) together with `"override_checks_state"` naming the state being overridden (`"pending"`, `"fail"` or `"none"`), in that one request's body, on the `expected_head` above. An override granted for "pending on A" therefore covers neither "fail on A" nor anything on B. Every checks refusal carries `checks` and `head_sha`. The override is never stored, and `force` does not imply it. The server logs each override, and it is recorded in the card history (`checks_overridden`) and in `integration_history` (`checks_at_merge`). The UI shows the check state in the Ship dialog and asks separately before sending the override. Do not assume the hosting platform enforces checks on its own; treat this board as the gate.
...
`GET /api/state`, `POST /api/backlog/refresh`, `POST /api/backlog/import` (JSON array), `GET /api/cards/{id}`, `GET /api/cards/{id}/log?tail=300`, `GET /api/cards/{id}/pr` (the card PR's verified `head_sha` and `checks`), `POST /api/cards/{id}/{develop|stop|feedback|accept|reject}` (accept takes `expected_head`, optional `force`, `override_checks` + `override_checks_state`), `POST /api/cards/{id}/blockers`, `POST /api/cards/{id}/blockers/{bid}/resolve`, `GET /api/integration` (cached for 20 s; a stale view only means the merge is refused as `head_moved`), `POST /api/integration/{open-pr|merge|new}` (merge takes `expected_head` like accept).
...
- commit on its branch and **not** push or open a PR: the board pushes the branch (non-force) and opens the PR against the integration branch when the run ends
- **always** write `.kanban/report.json`:
  `{status: done|blocked|failed, summary, pr_url, pr_number, blockers: [{text, severity}], verification}`. `pr_url`/`pr_number` are normally `null` now; the board finds or opens the PR itself.
...
- **The board cannot stop its own agents from merging. This is a known limitation, not a guarantee.** Dispatched agents run as **your user id**, without a container or a separate uid, and with a shell, under `--permission-mode $KANBAN_PERMISSION_MODE` (default `auto`). "Only a human merges to `main`" is therefore a convention the agents are told to follow and the board's own code keeps. It is not a boundary an agent cannot cross. A same-uid agent that decides to misbehave can still:
  ...
  - push straight to the base or integration branch with your git credentials. `Bash(git push:*)` denies every command that *starts* with `git push` (so `git push origin HEAD:main`, a trailing `--force`, `+src:dst` and `:dst` are all covered), and agents are told the board pushes for them. A prefix rule cannot see `git -C . push`, `env git push`, a `sh -c` wrapper or a script the agent writes and runs. **Add a server-side rule on the base branch and on `integration/*` that requires a pull request (and blocks force-pushes and deletion)**; do not rely on the token lacking merge rights alone, because a push needs no merge right;
  ...
  - read the pairing link from wherever the board's stdout goes (a terminal, tmux or the journal) while the link is still unredeemed. The scheduler starts no agent until you have paired, so cards left `queued` by the previous server wait for you. The one exception is an agent that was still *running* when the server restarted: it is reattached, not restarted, and the board logs one warning at startup, naming how many such agents are still running while the link is unredeemed. Pair promptly after a restart;
  - read `/proc/<pid>/environ` of any process of yours. The board therefore re-execs itself at startup without the names in the never-passed list above (`EPIGRAPH_TOKEN`, the gh tokens, `DATABASE_URL`, ...), handing their values to the new image over a pipe, so its own environ holds none of them. Two leftovers: while a board `gh` call runs, *that* `gh` process's environ holds the gh token it needs; and any other secret you export in the shell that starts the board stays readable, because the board cannot know it is one. Start the board from a shell that exports nothing else sensitive;
  ...
  What the hardening does buy: the session secret is not on disk, not in any environment, and not in `/api/state`; agents inherit no tokens from the environment, and the board's own `/proc/<pid>/environ` holds none of the never-passed names; helper agents have no shell at all; and agent-authored text cannot reach a prompt unquoted. Together these close the *accidental* paths and the cheap ones. **For real isolation, run the agents as a separate OS user or in a container that holds no GitHub credential with merge rights**, or at least give their `gh` a token that cannot merge to the base branch. Keep `KANBAN_MAX_AGENTS` low, and pick a permission mode you are comfortable with.
...
- **What the board does to your repository.** Your working tree is never edited: no files written, no branch switched by the board's own commands, and no exclude file touched. Agents and the retirement run work in worktrees under `$KANBAN_HOME/worktrees`, and the backlog fetch runs in `$KANBAN_HOME/helper-cwd`. The shared `.git` *is* used, because that is what linked worktrees are:
  ...
  Because the board's git gets no tokens, its pushes (the integration branch and each card's `kanban/*` branch) authenticate through a git credential helper, `gh auth setup-git`, or `SSH_AUTH_SOCK`. Every board push names an explicit refspec, so a planted `remote.<name>.mirror` cannot widen it (git refuses `--mirror` with refspecs; measured, nothing is pushed).
...
- `git push` denied to agents; the board pushes only the card branch, never forced (`BoardPushesTest`)
