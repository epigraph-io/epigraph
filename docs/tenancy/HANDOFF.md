...
**Status as of 2026-09-05.** PR-17 is the newest row. Everything below is local commits plus the merged integration branch; see `progress.json::prs.done` for the machine-readable ledger.
...
| PR-16 | ownership REQUIRED — drop defaults, arm trigger, validate (D1) | **done as 16a ONLY — the write-side gate is UNDELIVERED and needs a number.** Migrations **074/075/076**, not the plan's 070/071/072: all three of those are applied (070/071 are PR-12's, 072/073 are PR-13's), so editing them is a checksum mismatch, which **panics the api binary on restart**. `migrations/README.md` is authoritative and assigns 074/075/076; the 075/076 split is preserved deliberately so `claims`'s `VALIDATE CONSTRAINT` scan does not hold `SHARE UPDATE EXCLUSIVE` for the sum of every other tier-A table's. **Delivered (16a):** the defaults dropped on all 25 tier-A tables, the require-tenancy triggers given their final `RAISE` forms, `claims_block_widening`, the §4.6 INSERT call sites **plus 9 root-table statements the plan lists nowhere**, `consolidate`'s meet rule (stated by no prior plan revision — group beats public; a cross-group merge **refuses** with 409), `cargo sqlx prepare` with `.sqlx/` committed, `ci.yml` + `MIGRATION_DATABASE_URL`, two boot assertions, and `crates/epigraph-db/tests/tenancy_required.rs`. **NOT delivered (16b):** the SQL half of `viewer.writable_bind()` (the write-side predicate / `WITH CHECK`), the **35** fail-open scope sites across 8 files, the 16 MCP write tools among the 19 viewer-less ones, `theme_cluster`'s viewer scope, and the call-site lint. `viewer_route_table_lint.rs` and `tool_viewer_coverage.rs` are **both unmodified**, and `PolicyGate::authorize` still has **zero** production call sites — its group arm is still inert and still must not be read as dead code. The five `D-PR16-*` obligations are **re-owned onto the write-gate PR**, not discharged. **Corrections to the recon:** `viewer.writable_bind()` **already exists** (since PR-04; `visibility.rs` is untouched here) — what is missing is its SQL half, and Q7's rationale saying otherwise is amended in `progress.json`; the fail-open count is **35 across 8 files**, not 37 across 9 (PR-10 removed the `webhooks.rs` entry); and `epigraph-migrate.rs` **stays** on `no_unmaintained_dsn.rs`'s exemption list — that lint keys on pool CONSTRUCTION, so removing the entry is what turns CI red. **Deploy: three steps, do not compress** — (i) deploy the patched binaries with 074/075/076 **not applied**; (ii) watch `tenancy_undeclared_writes` flat at zero for 24 h, **after positively falsifying the counter on a canary** (`D-PR16-undeclared-write-counter-link-uncovered`); (iii) then 074, then 075, then 076. Skipping (ii) makes **every claim write raise `23502` the instant 074 commits**. 074 is a one-way door; undo script `docs/runbooks/074-undo.sql`. **The 16a/16b split is the operator's call** — it changes plan numbering, so nothing was renumbered here; the recommendation and its evidence are in `progress.json::prs.done[PR-16].the_16a_16b_split`. PR-17 depends only on 16a and is not blocked |
| PR-17 | RLS policies, FORCE, and a canary that proves it | **done** (code + migrations **077/078/079**, not the plan's 073/074/075 — 073 is PR-13's and 074/075/076 are PR-16's, all applied, and editing an applied version is a checksum mismatch that **panics the api binary on restart**; `migrations/README.md` is authoritative). **ENFORCEMENT BEGINS AT 077, NOT 079**: a policy already filters every non-owner without `BYPASSRLS`, and every role the app and job fleet connect as is one — 079 closes only the *owner* hole, which is why PR-15's probe was widened to `(relrowsecurity OR relforcerowsecurity)` and why anything keying on FORCE alone repeats a fixed bug. **The boot assertion is STAGED on `current_user = 'epigraph_app'`**, not on FORCE: the plan's Acceptance ("refuses if `relforcerowsecurity` is false") cannot hold, because at the moment §9.2 step 11d confirms the boot assertions the migrations have not run — an unstaged assertion bricks the rollout it exists to protect, the 077→079 window, and the documented `NO FORCE` kill switch. PR-16's `assert_tenancy_triggers_armed` is untouched. **Adversarial review found three policy arms written as "structural, self-certifying" predicates that referenced only ROW columns and therefore filtered nothing; all three are FIXED here, not deferred** — the provisioning bootstraps moved into two SECURITY DEFINER *writers* (`epigraph_ensure_personal_group`, `epigraph_provision_oauth_agent`, called by `agent.rs::ensure_personal_group` / `::ensure_for_client`), and `rls_enforcement.rs::no_policy_arm_is_session_independent` is the ratchet that makes the class unrepeatable. Two membership oracles were narrowed to the calling principal and a third deleted. Also fixed: an `INSERT … RETURNING` in `SecurityEventRepository::log` that RLS refuses (the SELECT policy applies to `RETURNING` rows — the audit trail would have stopped silently, since both callers swallow the error); a boot **probe** error that was fatal even though the **verdict** was staged; and `GRANT … ON ALL TABLES` missing every table created by a later migration on a fresh migrate (`webhook_subscriptions`, 085) — 077 now also issues `ALTER DEFAULT PRIVILEGES`. `current_user <> 'epigraph_app'` ships as a **WARN** (`D-PR17-current-user-refusal-left-as-a-warning`) because arming it makes the staging marker its own trigger; a seventh refusal (`epigraph_seed` membership) was added instead, discharging `D-PR16-seed-membership-refusal-downgraded`. `security_invoker = true` set on `alternative_set` and `alt_set_decisions`, discharging the obligation `public.tenancy_exempt` records against 077 by name. **Deliberately NOT done:** `no_unscoped_pool.rs` (466 raw `state.db_pool` sites and `acquire_as` has zero production callers — the lint would fail on day one; `D-PR17-request-path-never-stamps-session-gucs`), ops-F5 (its subject is PR-18's 080–083 and does not exist — a plan correction, not an omission), and `epigraph_app` **LOGIN**, which stays an out-of-band operator step because this repository is public. **This PR is inert on every environment that exists today**; it becomes load-bearing at 11d, whose precondition is now *asserted* by `tenancy_required.rs::a_derived_insert_is_refused_when_the_session_gucs_are_unstamped` rather than merely documented |
...
| PR-22 | retire the `ownership` table | **done** (migration **084**, not the plan's 080 — `migrations/README.md`'s documented +4 shift; 080 is PR-18a's `privatization_plans`, applied and frozen). Drops `public.ownership` and, EXPLICITLY and in their own statements, the `ownership_key_id_quarantine` VIEW and `public.epigraph_ownership_transcribe()` — `DROP TABLE` removes the two triggers but not 071's SECURITY DEFINER body, and a `CASCADE` would take the view the first pre-flight inspects. **Two `DO $$` pre-flights, both `RAISE EXCEPTION`**: an empty quarantine, and zero non-public rows without a `tenancy_transcription_log` entry. The plan's `DROP TRIGGER ownership_write_through` names a trigger that has never existed in any migration; the two real ones are `ownership_updated_at` (001) and `ownership_transcribe` (071). Rust half: deletes `repos/ownership.rs` + its two re-exports, retires `tenancy_backfill`'s `transcribe_legacy_ownership` pass and its two `ownership` `verify` checks — **only ONE of the two moved into the migration**: the `unlogged` check is pre-flight 2's predicate (superseded, then strengthened to require the ledger to record the partition the row CURRENTLY holds), while the OTHER check (non-public row → still-public claim) deliberately gets no pre-flight, because as one it would false-positive on a legitimate declassification; it is discharged by DEPLOY ORDER in `docs/deploy.md` steps 1-2 instead, and **rewrites all eight `ownership`-joining statements in `repos/structural.rs`**, discharging `D-PR22-structural-ownership-join` and closing `F-edge-count-double-counts`. **CONTRACT CHANGE, BOTH DIRECTIONS:** only `claims.agent_id` and `perspectives.owner_agent_id` survive as owner relations, so `evidence`, `community`, `context` and `frame` leave `node_counts` — but the DOMINANT direction is a **WIDENING**: nothing ever auto-populated `ownership` (zero `INSERT INTO ownership` in 90 migrations; its only writers died with PR-14), so the owned set goes from near-empty to the agent's entire authored corpus, and the `claims` arm has no `is_current` filter so node identity moves from per-lineage to per-VERSION. The direction is certain; the magnitude is exactly **M1**, still unmeasured — do not assume zero. The new key `claims.agent_id` is ALSO caller-supplied and **not a credential** (`routes/claims.rs` says so at length): attribution injection into a caller-facing read, not a confidentiality leak — every statement still ANDs the viewer predicate. Re-owned onto the write-side gate under `D-PR16-claim-authorship-is-not-a-credential`. **ONE-WAY DOOR**; undo at `docs/runbooks/084-undo.sql`, which recreates the empty shape and is explicit that the rows do not come back. New finding **`F-PR22-A`** at `migrations/076_validate_tenancy_remaining.sql` (second `DO` block), unassigned: a catalog guard there does not hold at head, so a MANUAL re-application of 076 against a database at head fails. 076 is applied and frozen, so it is not corrected. Analysis held outside this repository |
...
| # | Question | Gates |
|---|----------|-------|
| M1 | `ownership` row census | before PR-12; blocked PR-22. **PARTLY DISCHARGED BY PR-22, and partly not.** As a DROP PRECONDITION it is discharged by instrument rather than by measurement: migration 084's two pre-flights run the census on the database being deployed and REFUSE, which is stronger than a one-off count taken at some earlier date. As a CONTRACT question it is still open and now matters more — PR-22 narrows `GET /api/v1/structural-features/:owner_id` to the two node types that still have an owner relation, and M1 is precisely the measurement that would say whether any production owner holds `ownership` rows with `node_type` in ('evidence','community','context','frame') and therefore loses counts. Unmeasured; state it as contingent, do not assume zero |
...
- **PR-16 raised six**, all in `progress.json::deferred_obligations`, all with
  owners — they exist because a residual recorded only as a code comment is not
  a tracked commitment:
  `D-PR16-recall-events-are-instance-wide` (every agent's raw `query_text` is
  still readable by every authenticated agent — unchanged from migration 062's
  DEFAULT, but now a call-site decision the type's own doc contradicts),
  `D-PR16-undeclared-write-counter-link-uncovered` (074 removes 070 arm (a)'s
  counting limb, so the §9.2 step (ii) deploy gate's instrument is no longer
  test-covered — mitigated by a canary-falsification step in `docs/deploy.md`,
  not by a test),
  `D-PR16-seed-grant-to-the-harness-role` (the *Files* line's
  "`epigraph_seed` granted to the test harness pools" is **not implemented**;
  the suite reaches the hatch via superuser-only `SET SESSION AUTHORIZATION`),
